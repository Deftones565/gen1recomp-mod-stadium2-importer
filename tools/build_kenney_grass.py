...
def smooth_tree(rows, round_crown, subdivisions=2):
...
def main():
    ...
    for name in names:
        ...
        for line in (base / (source_name + '.obj')).read_text().splitlines():
            f = line.split()
            if not f:
                continue
            if f[0] == 'v':
        ...
        for material, triangles in materials.items():
            ...
            if name.startswith('tree_'):
                triangles = smooth_tree(triangles, foliage, 1 if name.endswith("_lod") else 2)
