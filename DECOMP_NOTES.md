...
`lib/model_handlers.lua` compiles these families into a portable `S2HX` trailer on every generated `DSM4` pack. DSM4 owns lossless core mesh semantics—geometry mode, vertex RGBA/normal interpretation, callback targeting, and sampler state—while the extension retains callback programs and decoded FRAGMENT data. Callback ownership and implementation live in the declarative handler registry and family modules. Descriptor `0x81000038` is implemented by `lib/render_callbacks/flame.lua`: `func_810059D0` loads one of eight IA16 source payloads and renders the same bytes through a 32x64 IA8 tile, while `func_80070974` renders the shared ten-vertex object at `0x8009F228` through the display list at `0x8009F2E0`; `func_80070A4C` supplies its default, Magmar, and Moltres color branches. Pose records are aligned with model-local auxiliary streams by authored order and duration in `animation_routing.lua`.
