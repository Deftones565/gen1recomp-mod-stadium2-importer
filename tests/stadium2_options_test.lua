...
local betaArena, betaTod, betaBattleFx = false, false, false
Importer.bind({ options={ get=function(_, key)
  ...
  if key == "stadium2_beta_battle_fx" then return betaBattleFx end
end } })
...
betaTod=false
ok(not Importer.betaBattleFxEnabled(), "Stadium 2 move FX remain default-OFF")
betaBattleFx=true
ok(Importer.betaBattleFxEnabled(), "beta move FX setting reaches the importer")
betaBattleFx=false
shader = "cel"
