-- Real Gen 2 encounter-to-arena visual integration test.
--
-- Run one case per LOVE process so battle transitions, mod events, arena
-- ownership and screenshot output all follow the same path as normal play:
--
--   STADIUM2_ARENA_VISUAL_CASE=outdoor_trainer \
--   POKEPORT_GAME=crystal \
--   POKEPORT_DRIVER=mods/STADIUM2_IMPORTER/tests/drivers/gen2_context_arena_visual.lua \
--   love .
--
-- The companion run_context_arena_visuals.sh runs every case. This driver
-- asserts the selected arena before capturing, so a screenshot of the wrong
-- field cannot be reported as a passing visual.
...
return function(game)
  local name=(os.getenv("STADIUM2_ARENA_VISUAL_CASE") or "wild"):lower()
  ...
  assert(status.betaArena~=case.classic,
    case.classic and "wild encounter entered arena scene mode"
      or "trainer encounter did not enter arena scene mode")
  assert(status.arenaIndex==case.arena,
    ("%s selected arena %s, expected %s"):format(name,
      tostring(status.arenaIndex),tostring(case.arena)))
  assert(status.arenaReason==case.reason,
    ("%s selected reason %s, expected %s"):format(name,
      tostring(status.arenaReason),case.reason))
  ...
  print(("[stadium2-arena-visual] PASS case=%s map=%s arena=%s reason=%s scene=%s full=%s")
    :format(name,case.map,tostring(status.arenaIndex),status.arenaReason,path,fullPath))
  love.event.quit()
end
