-- Run from the game root: love mods/STADIUM2_IMPORTER/tests/drivers/scenery_instancing_visual
...
function love.load()
 ...
 local shader=g.newShader(require("mods.STADIUM2_IMPORTER.lib.scenery_instances").shader([[
 varying vec3 shade;
 #ifdef VERTEX
 ...
 for _,name in ipairs({'nature','cave','town','freshwater'}) do
  ...
  print(name..' base vertices '..tostring(chunks.baseVertices)..' / '..#rows)
  ...
  for i=0,4 do
   ...
   -- GPU transforms and equal-depth overlap order can differ at a few edge pixels.
   assert(diff<=640*360*.001,'more than 0.1% of pixels changed; inspect instance transforms/materials')
   ai:release();bi:release()
  end
  chunks:release();original:release();scene.release()
 end
 -- Compile and execute the actual textured day/night and shadow shaders too.
 for _,name in ipairs({'nature','town','freshwater'}) do
  local scene=require('mods.STADIUM2_IMPORTER.lib.battle_'..name)
  scene.bind({read=function(_,path)
   local f=assert(io.open(love.filesystem.getWorkingDirectory()..'/mods/STADIUM2_IMPORTER/'..path,'rb'))
   local data=f:read('*a');f:close();return data
  end})
  local frame=Camera.frame(640,360)
  for _,day in ipairs({'DAY','NITE'}) do
   g.setCanvas({a,depth=true});g.clear(0,0,0,1,true,true)
   scene.castShadow(g,frame.vp)
   scene.draw(g,frame,{daytime=day,light={-.4,-.8,-.4},modelTint={1,1,1}})
   g.setCanvas();g.setShader()
   print(name..' '..day..' production shaders and instanced shadow draw passed')
  end
  scene.release()
 end
 end)
